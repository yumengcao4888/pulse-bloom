...
export async function DELETE() {
  try {
    ...
    const [reflectionsResult] = await prisma.$transaction([
      prisma.reflection.deleteMany({ where: { healerId: healer.id } }),
      prisma.healer.delete({ where: { clerkId: userId } }),
    ]);

    return NextResponse.json(
      { success: true, deletedReflections: reflectionsResult.count },
      { status: 200 },
    );
  } catch (err) {
    console.error("Error in /api/healer DELETE:", err);
    return NextResponse.json({ error: "Invalid request" }, { status: 500 });
  }
}
